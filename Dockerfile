...
#docker rm -f node-reptile-tokyo-mou ; docker run -it --privileged -v ~/docker/node-reptile-tokyo-mou:/node -p 1080:80 -e "DbHost=rm-2zew3g96vzbn5648c.mysql.rds.aliyuncs.com" -e "DbPort=3306" -e "DbUsername=psc" -e "DbPassword=psc2019%" -e "Database=pscdb" --name node-reptile-tokyo-mou --log-opt max-size=1m --log-opt max-file=1 land007/node-reptile-tokyo-mou:latest
